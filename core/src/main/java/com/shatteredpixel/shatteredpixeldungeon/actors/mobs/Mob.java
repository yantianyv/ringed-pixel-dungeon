...
public abstract class Mob extends Char {
    ...
    protected void onAdd() {
        if (firstAdded) {
            ...
            // 只有默认值（1）的普通怪物才会被影响，特殊怪物已经明确设置了num_of_escape，不受影响
            if (num_of_escape == 1 && alignment == Alignment.ENEMY) {
                float escapeChance = 0.10f + 0.03f * Dungeon.depth;
            ...
            if (buff(Amok.class) != null) {
                ...
                for (Mob mob : Dungeon.level.mobs) {
                    if (mob.alignment == Alignment.ENEMY && fieldOfView[mob.pos]
                            && mob.invisible <= 0 && !mob.isInvulnerable(getClass())) //do not target passive mobs
    ...
    protected class Escaping implements AiState {
        ...
        public boolean act(boolean enemyInFOV, boolean justAlerted) {
            enemySeen = enemyInFOV;
            if (Dungeon.level.locked || properties.contains(Property.STATIC)) {
                escaped();
                if (state != ESCAPING) {
                    spend(TICK);
                    return true;
