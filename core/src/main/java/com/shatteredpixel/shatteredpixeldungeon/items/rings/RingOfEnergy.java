...
public class RingOfEnergy extends Ring {
    ...
    public static float wandChargeMultiplier(Char target) {
        float bonus = (float) Math.pow(1.1, getBuffedBonus(target, Energy.class));
    ...
    public static float LevelUpBuff(Char target) {
        ...
        if (getBuffedBonus(target, Energy.class) > 0) {
            bonus = (float) Math.pow(1.1, getBuffedBonus(target, Energy.class));
        } else {
            return bonus;
        }
        return bonus;
    ...
    public static float armorChargeMultiplier(Char target) {
        return (float) Math.pow(1.1, getBuffedBonus(target, Energy.class));
    }
